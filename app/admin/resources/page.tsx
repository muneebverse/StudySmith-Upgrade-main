  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <Link href="/admin/resources/new" className="bg-black text-white rounded px-4 py-2 text-sm font-medium">
          + New Resource
        </Link>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        {resources && resources.length === 0 && (
          <p className="text-sm text-gray-500">No resources yet. Click &quot;New Resource&quot; to add one.</p>
        )}
                ...
                {resources.map((r) => (
                  <tr key={r.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link href={`/admin/resources/${r.id}`} className="text-sm font-medium hover:underline">Edit</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
